...
func (app *App) handleSearch(w http.ResponseWriter, r *http.Request) {
    ...
    results, err := app.search.Search(r.Context(), query)
    if err != nil {
    ...
    if err := app.conversations.StoreSearchResults(r.Context(), conversationID, results); err != nil {
...
func (service *ConversationService) StoreSearchResults(ctx context.Context, conversationID int64, results []SearchResult) error {
    tx, err := service.db.BeginTx(ctx, nil)
    if err != nil {
        return err
    }
...
func (service *ConversationService) StoreSummary(ctx context.Context, conversationID int64, url, summary, sourceText string) error {
    ...
    if _, err := tx.ExecContext(ctx, `DELETE FROM summaries WHERE conversation_id = ? AND url = ?`, conversationID, url); err != nil {
        return err
    }

    if _, err := tx.ExecContext(ctx, `
        INSERT INTO summaries (conversation_id, url, summary, source_text, status)
        VALUES (?, ?, ?, ?, 'ready')
    `, conversationID, url, summary, sourceText); err != nil {
        return err
    }
...
func (service *ConversationService) GetSummaries(ctx context.Context, conversationID int64) ([]SummaryRecord, error) {
    rows, err := service.db.QueryContext(ctx, `
        SELECT url, summary, source_text, status
        FROM summaries
        WHERE conversation_id = ?
    ...
    for rows.Next() {
        ...
        if err := rows.Scan(&item.URL, &item.Summary, &item.SourceText, &item.Status); err != nil {
            return nil, err
        }
